...
class CBB():
	...
	def load_data(self,min_games=10,min_avg_fp=0):
		c = db.cursor()
		self.X = []
		self.y = []
		...
		c.execute("""SELECT team from teams where fanduel is not null""")
		fanduel_teams = {}
		for team in c.fetchall():
			fanduel_teams[team['team']] = 0
		...
		for aday in self.daterange(date(2015,1,8),days['stop']+timedelta(1)):
			...
			(SELECT IF(team=games.home,games.away,games.home) opponent,season,team,games.gameid,pid,pts,reb,ast,blk,stl,tov,pos FROM gamelog,games WHERE games.gameid=gamelog.gameid AND date(games.gametime) = %s AND pos!='NA') AS today \
			WHERE hist.pid = today.pid order by today.gameid,today.pid;",
			(aday,aday,min_games,min_avg_fp,aday))

			for d in c.fetchall():
				if fanduel_teams.has_key(d['team']) and fanduel_teams.has_key(d['opponent']):
					fh = {}
					fh['pid'] = d['pid']
		...
		for row in self.feature_headers:
			...
			for game in result:
				try:
					...
					# d.append([float(game['fph']),float(game['min'])])
				except:
					print 'BAD MINUTES'
					mins = []
					for game in result:
					...
					# d.append([float(game['fph']),float(min_avg)])
	...
	def add_opponent_defense_limit_categories(self,n):
		...
		for row in self.feature_headers:
			...
			c.execute("""SELECT (SUM(pts)+SUM(reb)*1.2+SUM(ast)*1.5+SUM(blk)*2+SUM(stl)*2-SUM(tov))/SUM(min) fph,gametime FROM gamelog where gamelog.team != %s  and (home = %s or away = %s) and gametime < %s and gamelog.min is not null group by gameid order by gametime desc limit %s""",\
				(row['opponent'],row['opponent'],row['opponent'],row['gametime'],n))
			result = c.fetchall()
			d = []
			for game in result:
				d.append(float(game['fph']))
			team_stats.append(d)
			# print team_stats
